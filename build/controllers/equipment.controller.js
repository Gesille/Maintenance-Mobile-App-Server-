...
export const getAllEquipment = async (req, res, next) => {
    try {
        const equipment = await odooRequest("maintenance.equipment", "search_read", [[["active", "=", true]]], { fields: EQUIPMENT_FIELDS, order: "name asc" });
        res.status(200).json({
            success: true,
...
export const getEquipmentById = async (req, res, next) => {
    try {
        ...
        const equipment = await odooRequest("maintenance.equipment", "search_read", [[["id", "=", id]]], { fields: EQUIPMENT_FIELDS });
        if (!equipment?.length)
            return res
                .status(404)
...
const generateQRGrid = async (doc, items) => {
    const itemsPerRow = 3;
...
export const printAllQRPdf = async (req, res) => {
    try {
        const raw = await odooRequest("maintenance.equipment", "search_read", [[["active", "=", true]]], { fields: ["id", "name", "partner_ref"], order: "name asc" });
...
export const printSingleQRPdf = async (req, res) => {
    try {
        ...
        if (isNaN(equipmentId))
            return res.status(400).json({ success: false, message: "Invalid ID" });
        const raw = await odooRequest("maintenance.equipment", "search_read", [[["id", "=", equipmentId]]], { fields: ["id", "name", "partner_ref"] });
        if (!raw?.length)
...
export const scanEquipmentQR = async (req, res, next) => {
    try {
        ...
        if (isNaN(id))
            return res.status(400).json({ success: false, message: "Invalid ID" });
        const equipment = await odooRequest("maintenance.equipment", "search_read", [[["id", "=", id], ["active", "=", true]]], { fields: EQUIPMENT_FIELDS });
        if (!equipment?.length)
            return res.status(404).json({ success: false, message: "Equipment not found" });
...
export const createMaintenanceRequest = async (req, res, next) => {
    try {
        ...
        const equipment = await odooRequest("maintenance.equipment", "search_read", [[["id", "=", Number(equipmentId)], ["active", "=", true]]], { fields: EQUIPMENT_FIELDS });
        if (!equipment?.length)
            return res.status(404).json({ success: false, message: "Equipment not found" });
...
export const getEquipmentFields = async (req, res) => {
    try {
        const fields = await odooRequest("maintenance.equipment", "fields_get", [], { attributes: ["string", "type", "required"] });
