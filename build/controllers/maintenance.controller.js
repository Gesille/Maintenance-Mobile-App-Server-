...
import { getMaintenanceRequests, getRequestMessages, postRequestComment, } from "../services/maintenance.service.js";
import { odooRequest } from "../odoo/odoo.client.js";
import { MAINTENANCE_REQUEST_FIELDS } from "../@types/Maintenance.constants.js";
const VALID_STATES = ["new", "under_repair", "done", "cancel"];
// ─── Get all maintenance requests ─────────────────────────────────────────────
// GET /maintenance
// Returns: { requests[], stages[], total }
// Used by: WorkOrdersPage → useGetAllMaintenanceRequestsQuery
export const getAllMaintenanceRequests = async (_req, res, next) => {
    try {
...
export const getMaintenanceRequestDetail = async (req, res, next) => {
    try {
...
export const updateMaintenanceRequestStatus = async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ success: false, message: "Invalid ID" });
        ...
        const values = { stage_id: await resolveStageId(state) };
        ...
        const updated = await odooRequest("maintenance.request", "search_read", [[["id", "=", id]]], { fields: MAINTENANCE_REQUEST_FIELDS });
...
export const assignTechnicians = async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ success: false, message: "Invalid ID" });
...
export const getMaintenanceRequestMessages = async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ success: false, message: "Invalid ID" });
...
export const postMaintenanceRequestComment = async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ success: false, message: "Invalid ID" });
...
async function resolveStageId(state) {
    if (!_stageCache) {
    ...
    const first = _stageCache.values().next().value;
    if (first === undefined) {
...
function buildStageMap(stages) {
    ...
    for (const s of stages) {
        ...
        if (name.includes("new") || name.includes("open"))
            map.set("new", s.id);
        if (name.includes("repair") || name.includes("progress"))
            map.set("under_repair", s.id);
